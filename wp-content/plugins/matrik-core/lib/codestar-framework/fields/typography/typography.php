  ...
  class CSF_Field_typography extends CSF_Fields {
      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Family', 'matrik-core' ) .'</div>';
            echo $this->create_select( array( $this->value['font-family'] => $this->value['font-family'] ), 'font-family', esc_html__( 'Select a font', 'matrik-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Backup Font Family', 'matrik-core' ) .'</div>';
            echo $this->create_select( apply_filters( 'csf_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'matrik-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Style', 'matrik-core') .'</div>';
            echo '<select class="csf--font-style-select" data-placeholder="Default">';
            echo '<option value="">'. ( ! $this->chosen ? esc_html__( 'Default', 'matrik-core' ) : '' ) .'</option>';
            ...
            if ( ! empty( $args['extra_styles'] ) ) {
              ...
              echo ( ! $this->chosen ) ? '<div class="csf--title">'. esc_html__( 'Load Extra Styles', 'matrik-core' ) .'</div>' : '';
              $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'matrik-core' ) : esc_html__( 'Default', 'matrik-core' );
          ...
          if ( ! empty( $args['subset'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Subset', 'matrik-core' ) .'</div>';
            $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
            echo $this->create_select( $subset, 'subset', esc_html__( 'Default', 'matrik-core' ), $args['multi_subset'] );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Align', 'matrik-core' ) .'</div>';
            echo $this->create_select( array(
              'inherit' => esc_html__( 'Inherit', 'matrik-core' ),
              'left'    => esc_html__( 'Left', 'matrik-core' ),
              'center'  => esc_html__( 'Center', 'matrik-core' ),
              'right'   => esc_html__( 'Right', 'matrik-core' ),
              'justify' => esc_html__( 'Justify', 'matrik-core' ),
              'initial' => esc_html__( 'Initial', 'matrik-core' )
            ), 'text-align', esc_html__( 'Default', 'matrik-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Variant', 'matrik-core' ) .'</div>';
            echo $this->create_select( array(
              'normal'         => esc_html__( 'Normal', 'matrik-core' ),
              'small-caps'     => esc_html__( 'Small Caps', 'matrik-core' ),
              'all-small-caps' => esc_html__( 'All Small Caps', 'matrik-core' )
            ), 'font-variant', esc_html__( 'Default', 'matrik-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Transform', 'matrik-core' ) .'</div>';
            echo $this->create_select( array(
              'none'       => esc_html__( 'None', 'matrik-core' ),
              'capitalize' => esc_html__( 'Capitalize', 'matrik-core' ),
              'uppercase'  => esc_html__( 'Uppercase', 'matrik-core' ),
              'lowercase'  => esc_html__( 'Lowercase', 'matrik-core' )
            ), 'text-transform', esc_html__( 'Default', 'matrik-core' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Decoration', 'matrik-core' ) .'</div>';
            echo $this->create_select( array(
              'none'               => esc_html__( 'None', 'matrik-core' ),
              'underline'          => esc_html__( 'Solid', 'matrik-core' ),
              'underline double'   => esc_html__( 'Double', 'matrik-core' ),
              'underline dotted'   => esc_html__( 'Dotted', 'matrik-core' ),
              'underline dashed'   => esc_html__( 'Dashed', 'matrik-core' ),
              'underline wavy'     => esc_html__( 'Wavy', 'matrik-core' ),
              'underline overline' => esc_html__( 'Overline', 'matrik-core' ),
              'line-through'       => esc_html__( 'Line-through', 'matrik-core' )
            ), 'text-decoration', esc_html__( 'Default', 'matrik-core' ) );
            echo '</div>';
          }
        ...
        echo '<div class="csf--blocks csf--blocks-inputs">';
          ...
          if ( ! empty( $args['font_size'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Size', 'matrik-core' ) .'</div>';
          ...
          if ( ! empty( $args['line_height'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Line Height', 'matrik-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['letter_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Letter Spacing', 'matrik-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['word_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Word Spacing', 'matrik-core' ) .'</div>';
            echo '<div class="csf--input-wrap">';
        ...
        if ( ! empty( $args['color'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Font Color', 'matrik-core' ) .'</div>';
        ...
        if ( ! empty( $args['custom_style'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Custom Style', 'matrik-core' ) .'</div>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        wp_enqueue_script( 'csf-webfontloader', 'https://cdn.jsdelivr.net/npm/webfontloader@1.6.28/webfontloader.min.js', array( 'matrik-core' ), '1.6.28', true );
        ...
        if ( ! empty( $customwebfonts ) ) {
          $webfonts['custom'] = array(
            'label' => esc_html__( 'Custom Web Fonts', 'matrik-core' ),
            'fonts' => $customwebfonts
          );
        }

        $webfonts['safe'] = array(
          'label' => esc_html__( 'Safe Web Fonts', 'matrik-core' ),
        ...
        $webfonts['google'] = array(
          'label' => esc_html__( 'Google Web Fonts', 'matrik-core' ),
        ...
        wp_localize_script( 'matrik-core', 'csf_typography_json', array(
          'webfonts'      => $webfonts,
